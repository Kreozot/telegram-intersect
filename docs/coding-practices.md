...
Large composite components must delegate distinct visual or behavioral blocks to child components.
In particular, when one location conditionally renders different domain entities, workflow states, or
substantial alternative layouts, represent those branches as named components instead of embedding
large ternaries or fragments in the parent's JSX. Each extracted component lives in its own folder
with its colocated stylesheet and component-specific logic. Keep small conditional labels, controls,
and empty states inline when extraction would obscure rather than clarify ownership.
