...
The browser polls normalized Telegram and snapshot state only while login is in progress or a scan is
running. An idle authenticated workspace does not poll. User commands and returning to a visible tab
trigger one refresh.
...
- Login transitions and partial scan progress remain visible without manual reloads.
